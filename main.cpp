/*
 * g++ -I/mingw64/include/ncurses -o Progetto main.cpp player.hpp player.cpp box.hpp box.cpp -lncurses -lpthread -L/mingw64/bin -static
 *
...
void game_flow(int y_scr, int x_scr, WINDOW* map, bool next_lvl, class BOX box,
    ...
    if(cx==1){
    	...
    	{
    		clear();
    		mvprintw(1, 1, "game saved");
    		refresh();
    		getch();
    	}else
    	{
    		clear();
    		mvprintw(1, 1, "game could not be saved, oopsie :) ");
    		refresh();
    	    getch();
    	}
        endwin();
